...
module.exports = async function handler(req, res) {
  ...
  if (!fbUser) return res.status(401).json({ error: 'Not authenticated' });
  const userId = fbUser.uid;

  const sb = getSupabase();
  if (!sb) return res.status(200).json({ data: { wallets:[], btc:[], others:[], indian:[], intl:[], mf:[], banks:[] }, currency: 'INR', isNew: true });
  ...
  try {
    if (req.method === 'GET' && action === 'portfolio') {
      ...
      if (error && error.code === 'PGRST116') return res.status(200).json({ data: { wallets:[], btc:[], others:[], indian:[], intl:[], mf:[], banks:[] }, currency: 'INR', isNew: true });
    ...
    if (req.method === 'POST' && action === 'portfolio') {
      ...
      const { error } = await sb.from('portfolios').upsert({ user_id: userId, data: portfolio, currency }, { onConflict: 'user_id' });
      if (error) throw error;
    ...
    if (req.method === 'POST' && action === 'snapshot') {
      ...
      const { error } = await sb.from('snapshots').upsert({ user_id: userId, value_usd, breakdown, snapped_at: new Date().toISOString().slice(0,10) }, { onConflict: 'user_id,snapped_at' });
      if (error) throw error;
      return res.status(200).json({ ok: true });
    }
    ...
    if (req.method === 'GET' && action === 'snapshots') {
      ...
      const { data, error } = await sb.from('snapshots').select('value_usd, breakdown, snapped_at').eq('user_id', userId).gte('snapped_at', since).order('snapped_at', { ascending: true });
      if (error) throw error;
    ...
    if (req.method === 'POST' && action === 'alert_create') {
      ...
      const { data, error } = await sb.from('alerts').insert({ user_id: userId, type, asset, threshold, currency: currency||'USD', channel: channel||'email', telegram_chat_id }).select().single();
      if (error) throw error;
    ...
    if (req.method === 'POST' && action === 'share_create') {
      ...
      const { data, error } = await sb.from('share_links').insert({ user_id: userId, slug, show_values: show_values !== false }).select().single();
      if (error) throw error;
